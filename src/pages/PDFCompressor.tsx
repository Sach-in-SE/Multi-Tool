  ...
  const handleCompression = async () => {
    ...
    const compressedFiles = await Promise.all(
      files.map(async (file) => {
        const compressedBlob = new Blob([file.file.slice(0, file.size * (1 - compressionLevel / 100))], { type: 'application/pdf' });
    ...
    <div className="min-h-screen py-16 bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-gray-900 dark:to-indigo-900">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-lg shadow-xl p-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-8">PDF Compressor</h1>
          ...
          <div {...getRootProps()} className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors
            ...
            <input {...getInputProps()} />
            <FileText className="h-12 w-12 mx-auto mb-4 text-gray-600 dark:text-gray-400" />
            <p className="text-lg text-gray-600 dark:text-gray-300">Drag and drop PDFs here, or click to select files</p>
            <p className="text-sm mt-2 text-gray-500 dark:text-gray-400">Supports PDF - Max file size: 100MB</p>
          </div>
